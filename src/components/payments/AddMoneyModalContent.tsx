        ...
        if (res.ok && contentType.includes('application/json')) {
          ...
          if (isMounted) {
            setConfig({
              ...
              mode: (data.mode || 'test').toUpperCase() as 'TEST' | 'LIVE',
              configured: Boolean(data.configured),
              keyId: data.keyId || null,
              status: data.configured ? 'CONNECTED' : (data.status || 'NOT_CONFIGURED'),
          ...
          throw new Error('Payment API returned HTML instead of JSON. Check backend serverless routes.');
        } else {
          ...
          if (fallbackRes.ok && fbContentType.includes('application/json')) {
            ...
            if (isMounted) {
              setConfig({
                ...
                mode: (fbData.mode || 'TEST').toUpperCase() as 'TEST' | 'LIVE',
                configured: Boolean(fbData.configured),
                keyId: fbData.keyId || null,
                status: fbData.configured ? 'CONNECTED' : (fbData.status || 'NOT_CONFIGURED'),
                webhookUrl: fbData.webhookUrl || '',
                appUrl: fbData.appUrl || '',
                statusNote: fbData.statusNote || fbData.adminStatus || '',
                message: fbData.statusNote || '',
                missingVars: [],
              });
            }
          } else {
            throw new Error(`Could not load payment gateway configuration (HTTP ${res.status})`);
          }
        }
      } catch (err: any) {
        if (isMounted) {
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {config && (
              ...
              >
                {config.configured ? 'CONNECTED' : config.status}
              </span>
            )}
          </div>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="text-xs space-y-1">
                    ...
                    <p className="text-[11px] text-amber-300/80">
                      {config.message || 'Payment service requires configuration in backend environment variables.'}
                    </p>
                    {config.missingVars && config.missingVars.length > 0 && (
                      <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10px] text-amber-300">
                        <span className="font-medium text-amber-400">Missing in Vercel:</span>
                        {config.missingVars.map((v) => (
                          <code key={v} className="rounded bg-amber-900/60 px-1.5 py-0.5 font-mono text-amber-200 border border-amber-500/30">
                            {v}
                          </code>
                        ))}
                      </div>
